...
use crate::rest::config::{HermesOptions, BASE_URL};
use crate::rest::errors::HermesError;
...
pub struct RequestOptions {
    ...
    /// Query string parameters (`None` / omitted values are skipped).
...
/// Outbound request for a custom [`HttpBackend`].
#[derive(Debug, Clone)]
...
/// Inbound response for a custom [`HttpBackend`].
#[derive(Debug, Clone)]
...
pub trait HttpBackend: Send + Sync {
    ...
    fn execute<'a>(&'a self, req: HttpRequest) -> BoxFut<'a, Result<HttpResponse, HermesError>>;
}
    ...
    fn execute<'a>(&'a self, req: HttpRequest) -> BoxFut<'a, Result<HttpResponse, HermesError>> {
        Box::pin(async move {
            ...
            let res = builder.send().await.map_err(|e| {
                HermesError::new(e.to_string(), 0, "network_error")
            })?;
...
/// Calls `GET /auth/whoami` on construction and caches user/tenant.
pub struct HttpClient {
    ...
    /// Shared eager whoami future slot.
    identity_slot: Arc<Mutex<IdentitySlot>>,
}

enum IdentitySlot {
    Pending(tokio::sync::broadcast::Sender<Result<Identity, String>>),
    Done(Result<Identity, String>),
}

impl HttpClient {
    /// Construct with the default reqwest backend.
    pub fn new(api_key: impl Into<String>, options: HermesOptions) -> Result<Self, HermesError> {
        Self::with_backend(api_key, options, Arc::new(ReqwestBackend::new()))
    }
    ...
    pub fn with_backend(
        ...
        let client = Self {
            ...
            identity_slot: Arc::new(Mutex::new(IdentitySlot::Pending(tx))),
        };
        client.spawn_eager_whoami();
        Ok(client)
    }

    fn spawn_eager_whoami(&self) {
        let this = Self {
            api_base: self.api_base.clone(),
            api_key: self.api_key.clone(),
            backend: Arc::clone(&self.backend),
            identity_cache: Arc::clone(&self.identity_cache),
            identity_slot: Arc::clone(&self.identity_slot),
        };
        tokio::spawn(async move {
            let result = this.fetch_whoami().await.map_err(|e| e.to_string());
            if let Ok(ref id) = result {
                *this.identity_cache.lock().unwrap() = Some(id.clone());
            }
            let mut slot = this.identity_slot.lock().unwrap();
            match &*slot {
                IdentitySlot::Pending(tx) => {
                    let _ = tx.send(result.clone());
                    *slot = IdentitySlot::Done(result);
                }
                IdentitySlot::Done(_) => {}
            }
        });
    ...
    pub async fn whoami(&self) -> Result<Identity, HermesError> {
        ...
        let mut rx = {
            let slot = self.identity_slot.lock().unwrap();
            match &*slot {
                IdentitySlot::Done(Ok(id)) => return Ok(id.clone()),
                IdentitySlot::Done(Err(e)) => {
                    drop(slot);
                    // Retry once after a failed eager fetch.
                    return self.retry_whoami(e.clone()).await;
                }
                IdentitySlot::Pending(tx) => tx.subscribe(),
            }
        };

        match rx.recv().await {
            Ok(Ok(id)) => Ok(id),
            Ok(Err(e)) => self.retry_whoami(e).await,
            Err(_) => {
                // Sender dropped — retry.
                self.retry_whoami("whoami channel closed".into()).await
            }
        }
    }

    async fn retry_whoami(&self, _prev: String) -> Result<Identity, HermesError> {
        let id = self.fetch_whoami().await?;
        *self.identity_cache.lock().unwrap() = Some(id.clone());
        *self.identity_slot.lock().unwrap() = IdentitySlot::Done(Ok(id.clone()));
        Ok(id)
    }
    ...
    async fn fetch_whoami(&self) -> Result<Identity, HermesError> {
        ...
        let user = profile.user.filter(|s| !s.is_empty()).ok_or_else(|| {
            HermesError::new("whoami response missing user or tenant", 0, "invalid_identity")
        })?;
        let tenant = profile.tenant.filter(|s| !s.is_empty()).ok_or_else(|| {
            HermesError::new("whoami response missing user or tenant", 0, "invalid_identity")
        })?;
        let raw = serde_json::to_value(&WhoamiWire {
    ...
    async fn get_raw<T: DeserializeOwned>(&self, path: &str) -> Result<T, HermesError> {
        self.request_internal("GET", path, None::<&()>, &RequestOptions::default())
            .await
    }
    ...
    async fn request_internal<T: DeserializeOwned>(
        ...
        if res.status == 204 {
            return serde_json::from_value(Value::Object(Default::default())).map_err(|e| {
                HermesError::new(e.to_string(), 204, "decode_error")
            });
        }
    ...
    pub async fn get<T: DeserializeOwned>(
        ...
        self.request("GET", path, None::<&()>, options.unwrap_or(&RequestOptions::default()))
            .await
    }
    ...
    pub async fn post<T: DeserializeOwned>(
        ...
        self.request(
            ...
            path,
            Some(body),
            options.unwrap_or(&RequestOptions::default()),
        )
        .await
    }
...
pub fn list_query(
    ...
    if let Some(v) = group {
        q.insert("group".into(), v.into());
    }
    if let Some(v) = search {
        q.insert("search".into(), v.into());
    }
    RequestOptions {
        query: q,
        ..Default::default()
