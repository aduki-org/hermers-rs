...
const FORBIDDEN = [
  ...
  /crates\/(api|mail|dav|db|grpc|tenant|mcp)\//,
  /\*\*Sources?:\*\*/i,
];
...
function walk(dir, out = []) {
  ...
  for (const name of readdirSync(dir)) {
    if (name === 'node_modules' || name === '.git' || name === 'site' || name === 'target') continue;
...
console.log(`docs:check ok (${files.length} markdown file(s))`);
