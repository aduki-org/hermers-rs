...
```rust
let services = hermes.scheduling.services().await?;
let avail = hermes.scheduling.availability("2026-08-01T00:00:00", "2026-08-07T00:00:00").await?;
```
...
| `cancel_guest(token)` | `POST /book/guest/{token}/cancel` | `{ status: "canceled" }` |
