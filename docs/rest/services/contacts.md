...
hermes.ready().await?;
let created = hermes.contacts.create(ContactData {
...
let page = hermes.contacts.list(Some(Query { limit: Some(50), ..Default::default() })).await?;
```
...
| SDK | HTTP | Returns |
| --- | --- | --- |
| `create(data)` | `POST /user/contacts` | **`ContactModel`** (full row) |
| `list(query?)` | `GET /user/contacts` | `Page<Contact>` |
| `group(group)` | `GET /user/contacts/group/{group}` | `Page<Contact>` |
| `search(q)` | `GET /user/contacts/search/{q}` | `Page<Contact>` |
| `update_vcard` / `update_emails` / `update_phones` / `update_groups` / `update_meta` | `PATCH …/{hex}/…` | JSON **`null`** |
| `remove(hex)` | `DELETE /user/contacts/{hex}` | JSON **`null`** |
