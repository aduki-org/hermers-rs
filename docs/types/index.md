...
Canonical **wire JSON** shapes returned by the Hermers API. The Rust SDK maps these into `hermers::types` (REST) and `hermers::grpc::pb` (gRPC) — prefer these shapes when reading responses.
...
`hermers` (REST) throws `HermesError` after parsing this body. `hermers` (gRPC) throws `HermesGrpcError` with gRPC status names.
...
`hex`, `url`, `events` (jsonb), `active`, `created`, `tenant: { hex, name }` — no `secret`. Create response is `{ "hex": "…" }` (`Hex`). Field patches / delete return `{ "ok": true }` (`Ack`).
...
| Area | REST (`hermers` (REST)) | gRPC (`hermers` (gRPC)) |
| --- | --- | --- |
