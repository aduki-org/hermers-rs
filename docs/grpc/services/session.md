...
client.ready().await?;
let sessions = client.sessions.list(None, Some(20), None).await?;
```
...
| --- | --- | --- | --- |
| `whoami` | `() → Result<Session>` | `Whoami` | Session (from cache `raw` when available) |
| `load` | `(jti: string) → Result<Session>` | `Load` | Session |
| `revoke` | `(jti: string) → Result<void>` | `Revoke` | Empty |
| `list` | `(opts?) → Result<ListSessionsResp>` | `List` | Page of sessions |
